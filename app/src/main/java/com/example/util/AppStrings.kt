package com.example.util

import androidx.compose.runtime.compositionLocalOf
import com.example.data.model.AppLanguage
import com.example.data.model.CoinCategory

open class AppStrings(
    open val language: AppLanguage = AppLanguage.ENGLISH
) {
    // Navigation & Core
    open val navMarkets: String = "Markets"
    open val navFutures: String = "Futures"
    open val navMacro: String = "Macro"
    open val navSignals: String = "Signals"
    open val navLearn: String = "Learn"
    open val navSettings: String = "Settings"
    open val appTitle: String = "CryptoCycles"
    open val proBadge: String = "Pro"
    open val proActive: String = "PRO ACTIVE"
    open val backToFree: String = "Back to Free"
    open val upgradeToPro: String = "Upgrade to Pro"
    open val proRequired: String = "Pro Required"
    open val searchPlaceholder: String = "Search 100+ coins..."
    open val cryptocurrenciesCount: String = "CRYPTOCURRENCIES"
    open val cryptocurrencySingular: String = "CRYPTOCURRENCY"
    open val turboEngineActive: String = "Turbo Engine Active"
    open val rankPrefix: String = "Rank"
    open val closeButton: String = "Close"
    open val institutionalTerminal: String = "Professional Cycle & Derivatives Terminal"
    open val trustBarFeed: String = "Professional Feed: Direct Orderbook"
    open val latencyMs: String = "%dms latency"
    open val realtimeFeed: String = "REALTIME FEED"
    open val neutralPhase: String = "Neutral Phase"
    open val avg7d: String = "7D Avg"
    open val avg30d: String = "30D Avg"
    open val daysHigh: String = "d high"
    open val refreshTooltip: String = "Refresh market feeds"
    open val refreshingFeeds: String = "Refreshing market & futures data..."
    open val feedsUpdated: String = "Market feeds up to date"

    // Coin Categories
    open val catAll: String = "100+"
    open val catLayer1: String = "Layer 1"
    open val catLayer2: String = "Layer 2"
    open val catDefi: String = "DeFi"
    open val catAiInfra: String = "AI & Infra"
    open val catMeme: String = "Memes"
    open val catRwaDepin: String = "RWA & DePIN"
    open val catUtility: String = "Utility"
    open val catFavorites: String = "Favorites"

    // Markets Screen & Teasers
    open val marketsHeader: String = "Market Overview"
    open val marketsHeadline: String = "100+ Useful Real-World Assets & Cycle Blueprints"
    open val marketsSubtitle: String = "Where each coin is moving, past historical patterns, future projections & whitepapers."
    open val fromAth: String = "from ATH"
    open val tapForCycleAnalog: String = "Tap for cycle analog, projections & whitepaper"
    open val heroSubtitle: String = "HISTORICAL CYCLE ANALOG"
    open val currentPrice: String = "Current Price"
    open val analogMatch: String = "Analog Match"
    open val cyclePhase: String = "Cycle Phase"
    open val daysSinceAth: String = "Days since ATH"
    open val tapToViewCycles: String = "Tap to explore cycles & whitepaper"
    
    // Markets Screen Teaser Cards
    open val marketsOrderFlowTeaserTitle: String = "Order Flow & Futures Depth"
    open val marketsOrderFlowTeaserSummary: String = "Funding · OI · Liquidations 24h"
    open val marketsOrderFlowTeaserLocked: String = "Locked"
    open val marketsOrderFlowTeaserLockedDesc: String = "Unlock live leverage & liquidation flow"
    open val marketsOrderFlowTeaserUnlockedDesc: String = "Live DOM Ladder & Orderbook"
    open val marketsOrderFlowTeaserUpgrade: String = "Upgrade →"
    open val marketsOrderFlowTeaserOpenFutures: String = "Open Futures →"
    open val marketsSignalsTeaserTitle: String = "Whale Radar & Fractal Signals"
    open val marketsSignalsTeaserLiveTag: String = "LIVE FEED"
    open val marketsSignalsTeaserSample: String = "BTC: $148M net accumulation on Coinbase Prime"
    open val marketsSignalsTeaserAccumulationTag: String = "ACCUMULATION"
    open val marketsSignalsTeaserSub: String = "Liquidation maps & 2016/2020 cycle fractals"
    open val marketsSignalsTeaserOpen: String = "Open Signals Tab →"
    open val marketsMacroTeaserNextEvent: String = "Next Event: US Jobs (NFP) Friday 15:30 UTC"
    open val marketsMacroTeaserOpen: String = "Open Macro →"

    // Cycle Days & Historical Analog
    open val cycleDaysTeaserTitle: String = "Cycle Days & Bear Market Analog"
    open val cycleDaysTeaserBadge: String = "CYCLE CLOCK"
    open val cycleDaysTeaserDaysPostAth: String = "Day +332 post-ATH · ~43d to Macro Bottom Window"
    open val cycleDaysTeaserCompare: String = "Compare Day 330 with 2018 ($6.3k) & 2022 ($19.5k) →"
    open val cycleDaysCardTitle: String = "Cycle Days & Historical Analog"
    open val cycleDaysCardSubtitle: String = "Where price stood on this exact day in previous bear markets & why crashes occurred"

    // Historical Cycle Analog
    open val historicalAnalogHeader: String = "HISTORICAL CYCLE ANALOG"
    open val historicalAnalogProUnlockedTitle: String = "Pro Cycle Analysis Unlocked"
    open val historicalAnalogProTitle: String = "Historical Cycle Analog"
    open val historicalAnalogProUnlockedDesc: String = "Full historical analog curves & price targets are active."
    open val historicalAnalogProDesc: String = "Unlock matching dates & trajectory projections for all coins."
    open val historicalAnalogManagePro: String = "Manage Pro"
    open val historicalAnalogProPrice: String = "Unlock Pro (€3.99/mo)"
    open val movementAnalysisHeader: String = "MOVEMENT ANALYSIS & CYCLE HISTORY"
    open val whereItMovesNowLabel: String = "Where it moves now:"
    open val whereItMovedPastLabel: String = "Where it moved in past cycles:"
    open val nextMoveHeader: String = "NEXT PREDICTED MOVE (HISTORICAL ANALOGS)"
    open val cycleFractalLabel: String = "Cycle Fractal Day"
    open val historicalMatchLabel: String = "Historical Cycle Pattern Match"
    open val cycleChartTapToInspect: String = "Tap or drag chart to inspect cycle days & prices"
    open val cycleChartDayLabel: String = "Day"
    open val cycleChartFractalScore: String = "Fractal Match"
    open val cycleChartModeAll: String = "All Cycles"
    open val cycleChartModeProjection: String = "Projected Corridor"
    open val cycleChartMode2020: String = "2020-24"
    open val cycleChartMode2016: String = "2016-20"
    open val cycleChartMilestoneHalving: String = "Halving"
    open val cycleChartMilestoneBreakout: String = "ATH Breakout"
    open val cycleChartMilestonePeak: String = "Blow-Off Peak"
    open val cycleChartMilestoneFloor: String = "Cycle Floor"
    open val cycleChartCurrentPoint: String = "Current Position"
    open val cycleChartProjectedPath: String = "Projected Fractal Path"
    open val cycleChartProjectedZone: String = "Confidence Corridor"
    open val target1w: String = "1 Week"
    open val target2w: String = "2 Weeks"
    open val target4w: String = "4 Weeks"
    open val strategyGuideTitle: String = "📖 Strategy Guide: When to Buy & Sell"
    open val buyMomentTitle: String = "🟢 When is the Best Time to BUY (Accumulation)"
    open val buyMomentRules: String = "1. When Bitcoin price touches or approaches the 200W MA Floor (<15% gap). This marks historical cycle bottoms.\n2. When 111 DMA is far below (>40%) 350 DMA x 2. Minimum downside risk with maximum reward (DCA In)."
    open val sellMomentTitle: String = "🔴 When is the Best Time to SELL (Cycle Peak)"
    open val sellMomentRules: String = "1. When 111 DMA crosses above 350 DMA x 2. This signals high probability of a cycle top.\n2. When Rainbow Chart enters the red 'Maximum Bubble' zones. Historical data shows massive drawdowns follow (90%+ for alts)."
    open val rainbowFeatureLabel: String = "🌈 Bitcoin Rainbow Chart (Dynamic Power Law Model)"
    open val altSeasonFeatureLabel: String = "⚡ Altcoin Season Index (0-100) & Top 50 Outperformers"
    open val piCycleFeatureLabel: String = "🎯 Pi Cycle Top Indicator & 200-Week MA Floor (Buy/Sell Rules)"
    open val fearGreedFeatureLabel: String = "😱 Fear & Greed Sentiment Multi-Timeframe Analytics"
    open val proAnalyticsTitle: String = "Pro Cycle Analysis"
    open val proAnalyticsDesc: String = "Unlock full 2020 & 2016 curve comparisons and predictive projections for"
    open val proAnalyticsButton: String = "Unlock Pro (€3.99/mo)"
    open val projectedPeakLabel: String = "2021 Multiplier Scenario (Historical reference only)"
    open val projectedBottomLabel: String = "Lower Cycle Zone (Historical)"

    // Market Stats & Token Supply
    open val marketStatsHeader: String = "MARKET STATS & TOKEN SUPPLY"
    open val circulatingSupplyLabel: String = "Circulating Supply:"
    open val circulatingShort: String = "Circulating"
    open val infiniteDeflationary: String = "Infinite (Deflationary)"
    open val marketCapMetric: String = "MARKET CAP"
    open val volume24hMetric: String = "24H VOLUME"
    open val tradingVolumeSub: String = "Trading volume"
    open val fdvMetric: String = "FDV (VALUATION)"
    open val fdvSub: String = "Fully diluted value"
    open val athMetric: String = "ALL-TIME HIGH"
    open val atlMetric: String = "ALL-TIME LOW"
    open val drawdownAthMetric: String = "ATH DRAWDOWN"
    open val fromPeakSub: String = "From apex peak"
    open val allTimeLowMetric: String = "All-time low"
    open val allTimeHighMetric: String = "All-time high"
    open val startMetric: String = "Start"
    open val liveYearsMetric: String = "6443d · 17.64yr live"
    open val daysSinceHighMetric: String = "Days since the high"
    open val dropFromHighMetric: String = "-36.0% drop from the high"

    // Whitepaper & Technology Blueprint
    open val whitepaperSectionHeader: String = "WHITEPAPER & TECHNOLOGY BLUEPRINT"
    open val founderLabel: String = "CREATOR / FOUNDER"
    open val genesisDateLabel: String = "GENESIS DATE"
    open val whatCoinDoesLabel: String = "What the coin does (Core Whitepaper Blueprint)"
    open val consensusLabel: String = "Consensus Mechanism & Architecture"
    open val tokenomicsLabel: String = "Tokenomics, Halving & Emission"
    open val useCasesLabel: String = "Core Ecosystem Use Cases"

    // Cycle Analysis Deep Dive
    open val weekOfFallTitle: String = "Historically bearish matching week (Sample: n=2 cycles)"
    open val weekOfFallSub: String = "week 46 · typically 16 wk"
    open val cycleAnalysisHeader: String = "Cycle analysis"
    open val fromHighToLowTitle: String = "FROM THE HIGH TO A POSSIBLE LOW"
    open val daysAfterAthLabel: String = "days after the all-time high"
    open val daysToBottomLabel: String = "days to a possible bottom"
    open val highMarker: String = "High"
    open val typical383dMarker: String = "typical 383d"
    open val lowMarker: String = "Low"
    open val fromHighToLowNarrative: String = "We are 319 days after the all-time high. Historically, from the high to the bottom took about 383 days. So we are about 64 days from a possible bottom."
    open val fromHighToLowDetails: String = "06 Oct 2025 · \$124,752.53 · -36.0% from the high"
    open val fromHighToLowDisclaimer: String = "This is what past cycles did. It is not a promise — only a clock."
    open val thisCycleVsLastTitle: String = "THIS CYCLE VS THE LAST ONES"
    open val daysAfterHighTitle: String = "Days after the high"
    open val daysAfterHighSub: String = "We are 319 days after the high. Past falls to a bottom lasted about 383 days."
    open val nowLabel: String = "Now"
    open val daysOfRiseTitle: String = "Days of a rise, historically"
    open val daysOfRiseSub: String = "Past rises from a low to the next high lasted about 1059 days."
    open val halvingTitle: String = "BITCOIN HALVING"
    open val halvingDaysLabel: String = "days to next halving"
    open val halvingHoursLabel: String = "hours remaining"
    open val whatCameNextTitle: String = "What happened on matching days in prior cycles"
    open val timeframe1wk: String = "in 1 wk"
    open val timeframe2wk: String = "in 2 wk"
    open val timeframe4wk: String = "in 4 wk"
    open val winRate71: String = "10 of 14 times it rose (historical reference)"
    open val winRate79: String = "12 of 15 times it rose (historical reference)"
    open val winRate86: String = "13 of 15 times it rose (historical reference)"
    open val whyWeSayThisTitle: String = "Historical Context & Cycle Caveats"
    open val whyPoint1: String = "This week of the cycle looks like 28 Jun 2018 — same phase, similar move."
    open val whyPoint2: String = "We looked at 14 similar weeks in the past. Over the next 4 weeks the price rose 12 of 14 times (typically +29.5%)."
    open val whyPoint3: String = "We are in week 46 of a fall. In the past a phase like this lasted about 16 weeks."
    open val whyPoint4: String = "Both the cycle position and the next weeks in the past point to the green zone."

    // Signals Screen
    open val signalsCoinSelectorTitle: String = "Cycle Asset"
    open val signalsAllCoins: String = "All 100+ Coins"
    open val timeframe1D: String = "1D"
    open val timeframe1W: String = "1W"
    open val timeframe1M: String = "1M"
    open val timeframe1Y: String = "1Y"
    open val timeframeCycle: String = "Macro Cycle"
    open val signalBullish: String = "BULLISH EXPANSION"
    open val signalBearish: String = "BEARISH CORRECTION"
    open val signalAccumulation: String = "ACCUMULATION BREAKOUT"
    open val signalConfidence: String = "Historical Reference Frequency"
    open val signalLiveFeed: String = "Live CoinGecko Feed • 1s Ticks"
    open val analogComparisonHeader: String = "HISTORICAL PRICE ACTION OVERLAY"
    open val analogExplanation: String = "Historical price action alignment with matching prior cycle dates. Not a prediction."
    open val highAccuracyNotice: String = "Historical Backtest Reference Log"

    // Settings Screen
    open val settingsHeader: String = "Settings"
    open val languageTitle: String = "Language"
    open val languageDesc: String = "Select your preferred language"
    open val appearanceTitle: String = "Appearance"
    open val themeGalaxyDark: String = "Galaxy Dark (Default)"
    open val themeGalaxyDarkSub: String = "Deep blue OLED background with cyan accents"
    open val themePureBlack: String = "Pure Black"
    open val themePureBlackSub: String = "True black OLED mode for maximum battery life"
    open val themeCleanWhite: String = "Clean White"
    open val themeCleanWhiteSub: String = "Crisp light theme for bright daylight environments"
    open val optionsTitle: String = "Options"
    open val soundEffectsTitle: String = "Crypto Sound Effects (SFX)"
    open val soundEffectsSub: String = "Haptic feedback on navigation & alerts"
    open val soundTestTitle: String = "Test Audio FX"
    open val soundTestLaunch: String = "Major Scale Ascent 🎵"
    open val soundTestClick: String = "Tech Tap"
    open val soundTestToggle: String = "Toggle Chirp"
    open val soundTestChime: String = "Coin Reward Chime"
    open val hideMemecoinsTitle: String = "Hide Memecoins"
    open val hideMemecoinsSub: String = "Only show high-market-cap fundamentally backed assets"
    open val logChartsTitle: String = "Logarithmic Charts"
    open val logChartsSub: String = "Display historical prices on log scaling to normalize percentage changes"
    open val proMembershipTitle: String = "Pro Membership"
    open val proMembershipPrice: String = "€3.99 / month · €34.99 / year"
    open val proBenefit1: String = "Institutional Masterclass: Chapters 18-22 (Macro, On-Chain & Quants)"
    open val proBenefit2: String = "Full Futures Signals Engine & Whale Leverage Radar"
    open val proBenefit3: String = "Pi Cycle Top/Bottom Exhaustion Models & Forward Audit Trail"
    open val proBenefit4: String = "Live Coin-Specific News Stream & Real-Time Alpha"
    open val proBenefit5: String = "Unlimited Instant Push Alerts & Deep Historical Cycle Analogs"
    open val currencyDisplayTitle: String = "Currency Display"
    open val howWeReadItTitle: String = "How we read it — in plain words"
    open val plainItem1Title: String = "1. History repeats in rhythms, not guarantees"
    open val plainItem1Desc: String = "Crypto follows roughly 4-year halving cycles. We align current price action with the same day in 2020 and 2016."
    open val plainItem2Title: String = "2. Historical analogs"
    open val plainItem2Desc: String = "The orange curve shows what happened in 2020; the cyan curve shows 2016. Your current trajectory is in solid white."
    open val plainItem3Title: String = "3. Historical Cycle Zones"
    open val plainItem3Desc: String = "Using logarithmic regression and diminishing return curves, we calculate past upper and lower zones for study. All tools are educational and not financial advice."
    open val plainItem4Title: String = "4. Fundamental Whitepaper Blueprints"
    open val plainItem4Desc: String = "Every coin profile includes its verified consensus model, tokenomics, founder origins, and real-world utility architecture."
    open val plainItem5Title: String = "5. Risk Disclaimer"
    open val plainItem5Desc: String = "Past performance is no guarantee of future returns. Crypto markets are volatile. Never invest funds you cannot afford to lose."
    open val privacyPolicyTitle: String = "Terms of Use & Privacy Policy"
    open val privacyPolicyView: String = "View our data collection & storage policy"

    // Pro Modal & Subscriptions
    open val proModalTitle: String = "CryptoCycles Pro"
    open val proModalSubtitle: String = "Unlock cycle analogs, alerts, and full coin analytics. Cancel anytime."
    open val proModalFeature1: String = "Institutional Masterclass: Chapters 18-22 (Macro, On-Chain & Quants)"
    open val proModalFeature2: String = "Full Futures Signals Engine & Whale Leverage Radar"
    open val proModalFeature3: String = "Pi Cycle Top/Bottom Exhaustion Models & Forward Audit Trail"
    open val proModalFeature4: String = "Live Coin-Specific News Stream & Real-Time Alpha"
    open val proModalFeature5: String = "Unlimited Instant Push Alerts & Deep Historical Cycle Analogs"
    open val proModalFeature6: String = "Unlimited Instant Push Alerts & Deep Historical Cycle Analogs"
    open val planMonthlyTitle: String = "Monthly"
    open val planMonthlyPrice: String = "€3.99"
    open val planMonthlyPeriod: String = "/ month"
    open val planMonthlyBadge: String = "7-Day Free Trial"
    open val planAnnualTitle: String = "Yearly"
    open val planAnnualPrice: String = "€34.99"
    open val planAnnualPeriod: String = "/ year"
    open val planAnnualBadge: String = "Launch 40% Off • ~€2.91/mo"
    open val subscribeProBtn: String = "Start 7-Day Free Trial"
    open val startFreeTrialBtn: String = "Start 7-Day Free Trial"
    open val startFreeTrialSub: String = "7 days free, then €3.99 / month. Cancel anytime."
    open val continueYearlyBtn: String = "Continue with Yearly (€34.99/yr)"
    open val continueYearlySub: String = "Instant Pro Access • €34.99 billed annually (~€2.91/mo)"
    open val restorePurchases: String = "Restore Purchases"
    open val restoringPurchases: String = "Restoring purchases from Google Play..."
    open val cancelAnytime: String = "Cancel anytime in Google Play Subscriptions"
    open val playTermsDisclosure: String = "Payment is charged to your Google Play account upon purchase confirmation. Subscriptions auto-renew unless canceled at least 24 hours prior to the end of the current period in Google Play Subscriptions. Manage or cancel anytime in Google Play Store > Subscriptions."
    open val financialDisclaimerTitle: String = "Financial & Regulatory Disclaimer"
    open val financialDisclaimerText: String = "CryptoCycles is provided strictly for educational, analytical, and historical purposes. All cycle tools, Rainbow projections, and halving analog overlays are educational and historical, NOT financial or investment advice. We do NOT promise or guarantee cycle tops, climax price targets, or algorithmic target price models. Digital asset markets carry high volatility and substantial capital risk. Never invest funds you cannot afford to lose. Always perform your own research (DYOR)."
    open val privacySheetTitle: String = "CryptoCycles Privacy"
    open val privacyLastUpdated: String = "Last updated: 2026"
    open val privSection1Title: String = "1. Data Collection & Analytics Privacy"
    open val privSection1Body: String = "CryptoCycles does not collect, sell, or monetize personal identifying information. Market metrics, cycle overlays, and order book telemetry are aggregated anonymously from public blockchain and exchange feeds. All technical indicators and cycle charts are strictly for educational and analytical reference and do not constitute financial advice."
    open val privSection2Title: String = "2. Children's Privacy"
    open val privSection2Body: String = "CryptoCycles is not directed at children under 13 (or the equivalent age in your country)."
    open val privSection3Title: String = "3. Contact & Inquiries"
    open val privSection3Body: String = "For a privacy request, use the contact email on the CryptoCycles Google Play listing. We will respond within 30 days."
    open val privSection4Title: String = "4. Local-First Architecture"
    open val privSection4Body: String = "CryptoCycles does not create user accounts and does not sell personal data. Themes, favorites, and Pro states are persisted locally on your device."

    // Signals Pro Locked Banner
    open val signalsProLockedTitle: String = "PRO Macro Cycle Signals & Halving Waves"
    open val signalsProLockedSubtitle: String = "Unlock institutional-grade macro cycle blueprints, 2020 & 2016 historical halving analogs, and real-time whale radar alerts."
    open val signalsProLockedFeature1: String = "Historical Halving Cycle Analogs (2020 & 2016 wave overlays)"
    open val signalsProLockedFeature2: String = "Grok ZigZag predictive peak & drawdown turning point algorithms"
    open val signalsProLockedFeature3: String = "Apex target & bottom accumulation timing windows"
    open val signalsProLockedFeature4: String = "Real-time Whale Alert Push Notifications for market-moving $10M+ orders"

    // Zone Playbook Strings
    open val zonePlaybookTitle: String = "CYCLE ZONE PLAYBOOK"
    open val zonePlaybookSub: String = "Educational playbook for managing risk based on macro cycle zones"
    open val zoneGreenAction: String = "Green Zone • Scale In (Accumulate)"
    open val zoneGreenDesc: String = "Historically favorable risk/reward band. Systematic dollar-cost averaging into core assets."
    open val zoneYellowAction: String = "Yellow Zone • Hold & Rebalance"
    open val zoneYellowDesc: String = "Mid-cycle valuation band. Maintain existing positions, avoid emotional chasing, rebalance if needed."
    open val zoneRedAction: String = "Red Zone • Scale Out (Reduce Risk)"
    open val zoneRedDesc: String = "Extended overbought valuation band. Historically prudent to take partial profits and secure capital."
    open val zonePlaybookDisclaimer: String = "For educational and study purposes only. Not financial advice."

    // Weekly Briefing Strings
    open val weeklyBriefingTitle: String = "WEEKLY MACRO BRIEFING"
    open val weeklyBriefingSub: String = "Weekly institutional cycle synthesis & risk posture"
    open val weeklyBriefingCurrentZone: String = "Current Macro Zone"
    open val weeklyBriefingActionTitle: String = "Recommended Posture"
    open val weeklyBriefingActionAccumulate: String = "ACCUMULATE"
    open val weeklyBriefingActionHold: String = "HOLD"
    open val weeklyBriefingActionReduce: String = "REDUCE"
    open val weeklyBriefingReasonTitle: String = "3-Line Synthesis"
    open val weeklyBriefingLine1: String = "1. Macro valuation metrics remain positioned in the lower accumulation corridor."
    open val weeklyBriefingLine2: String = "2. 200-week SMA floor and halving supply dynamics continue to provide long-term structural support."
    open val weeklyBriefingLine3: String = "3. On-chain wallet accumulation indicates steady absorption during consolidation phases."
    open val weeklyBriefingProUnlockText: String = "Unlock institutional Weekly Briefing, cycle playbook, and macro analogs with Pro."

    // Historical Analog Card Strings
    open val historicalAnalogCardTitle: String = "HISTORICAL CYCLE ANALOG"
    open val historicalAnalogPhaseText: String = "We are in the same cycle phase as %s."
    open val historicalAnalogFollowThrough: String = "After that point, typical move was: 30d: %s • 90d: %s • 180d: %s"
    open val historicalAnalogNotFinancialAdvice: String = "Educational cycle model based on 2016 & 2020 fractal history. Not financial advice."
    open val historicalAnalogProUnlockText: String = "Unlock historical analog fractal curves and 30/90/180-day trajectory paths."

    // Push Alerts Settings Strings
    open val alertSettingsHeader: String = "Cycle & Indicator Push Alerts"
    open val alertSettingsSub: String = "Instant on-device notifications for critical market cycle thresholds"
    open val alertZoneChangeTitle: String = "Zone Change Alert"
    open val alertZoneChangeDesc: String = "Notify when Bitcoin moves between Green, Yellow, and Red zones"
    open val alertPiCycleTitle: String = "Pi Cycle Top Gap & Cross"
    open val alertPiCycleDesc: String = "Notify when 111DMA and 350DMA x 2 convergence gap narrows or crosses"
    open val alertRainbowTitle: String = "Rainbow Band Migration"
    open val alertRainbowDesc: String = "Notify when Bitcoin transitions between Rainbow valuation bands"
    open val alert200WTitle: String = "200-Week SMA Distance"
    open val alert200WDesc: String = "Notify when price approaches or deviates significantly from 200W SMA"
    open val alertFreeLimitNotice: String = "Free tier includes up to 2 active alerts. Upgrade to Pro for unlimited alerts."
    open val alertProUnlockNotice: String = "PRO feature: Unlimited cycle, Pi Cycle, and Rainbow band push alerts."

    // Widget Strings
    open val widgetSettingsTitle: String = "Home Screen Widget"
    open val widgetSettingsDesc: String = "Display current cycle zone, BTC price, and days since ATH on your home screen"
    open val widgetZoneLabel: String = "Zone"
    open val widgetBtcPriceLabel: String = "BTC Price"
    open val widgetDaysSinceAthLabel: String = "Days Since ATH"

    // Whale Radar & Live Order Flow
    open val whaleRadarTitle: String = "WHALE RADAR & LARGE ON-CHAIN MOVES (PRO)"
    open val whaleRadarSub: String = "Real-time telemetry tracking institutional wallet and exchange flows (>$10M+)"
    open val whaleNotificationsTitle: String = "Whale Alert Push Notifications"
    open val whaleNotificationsDesc: String = "Receive instant push alerts when massive whale buys or exchange dumps occur."
    open val whaleNotificationsProLockedDesc: String = "Exclusive PRO feature. Real-time push alerts for market-moving whale transfers ($10M+)."
    open val whaleThresholdTitle: String = "Whale Alert Minimum Size"
    open val whaleFilterAll: String = "All Whales"
    open val whaleFilterInflow: String = "🚨 Inflows / Dumps"
    open val whaleFilterOutflow: String = "🟢 Accumulation"
    open val whaleFilterBuy: String = "🚀 Mega Buys"
    open val whaleCollapse: String = "Collapse"
    open val whaleViewAll: String = "View all (%d)"
    open val timeJustNow: String = "Just now"
    open val timeMinsAgo: String = "%dm ago"
    open val timeHoursAgo: String = "%dh ago"
    open val liveOrderFlowTitle: String = "24/7 LIVE ORDER FLOW (BINANCE)"
    open val liveOrderFlowSub: String = "Real-time tick stream: Buy vs Sell pressure oscillator"
    open val buyDominanceLabel: String = "BUY PRESSURE"
    open val sellDominanceLabel: String = "SELL PRESSURE"
    open val netVolumeDelta: String = "NET FLOW 24H"
    open val liveExecutionFeed: String = "LIVE EXECUTIONS"

    // Macro Screen Strings
    open val macroScreenTitle: String = "Macro & Cycle Indices"
    open val macroScreenSubtitle: String = "Professional valuation models, Rainbow power law & market sentiment"
    open val macroUpdated4m: String = "Updated 4m ago"
    open val macroProLockedTitle: String = "PRO Macro Cycle Suite & Rainbow Chart"
    open val macroProLockedSubtitle: String = "Unlock dynamic Bitcoin Power-Law Rainbow valuation, 90-day Altcoin Season Index, Fear & Greed sentiment analytics, and Pi Cycle Top indicators."

    // Macro Filters
    open val macroFilterAll: String = "All Macro"
    open val macroFilterEconomicCalendar: String = "📅 Jobs & CPI Calendar"
    open val macroFilterRiskBackdrop: String = "🌐 DXY, 10Y & FedWatch"
    open val macroFilterRainbow: String = "🌈 Rainbow Chart"
    open val macroFilterAltSeason: String = "⚡ Alt Season"
    open val macroFilterFearGreed: String = "😱 Fear & Greed"
    open val macroFilterPiCycle: String = "🎯 Pi Cycle & 200W"

    // Macro Economic Calendar & Risk Cards
    open val macroEconomicCalendarCardTitle: String = "📅 Economic Prints & NFP / CPI"
    open val macroEconomicCalendarCardSub: String = "Next: US Jobs (NFP) Friday 15:30 UTC"
    open val macroNfpFridayBadge: String = "NFP FRIDAY"
    open val macroRiskBackdropCardTitle: String = "🌐 DXY, 10Y Yields & FedWatch"
    open val macroRiskBackdropCardSub: String = "Global USD Liquidity & Sovereign Yields"
    open val macroRiskBackdropBadge: String = "MACRO BACKDROP"
    open val macroRainbowCardTitle: String = "🌈 Bitcoin Rainbow Price Model"
    open val macroRainbowCardSub: String = "Logarithmic Valuation Bands"
    open val macroRainbowBadge: String = "HOLD / DCA"
    open val macroAltSeasonCardTitle: String = "⚡ Altcoin Season Index"
    open val macroAltSeasonCardSub: String = "42/100 · Neutral Zone (Bitcoin Season)"
    open val macroFearGreedCardTitle: String = "😱 Fear & Greed Index"
    open val macroFearGreedCardSub: String = "68/100 · Greed Sentiment"

    open val macroUsJobsTitle: String = "US Jobs: Non-Farm Payrolls (NFP)"
    open val macroUsJobsTime: String = "Friday 15:30 UTC"
    open val macroCpiTitle: String = "US Consumer Price Index (CPI)"
    open val macroCpiTime: String = "Next Wed 13:30 UTC"
    open val macroForecastLabel: String = "Forecast"
    open val macroPreviousLabel: String = "Previous"
    open val macroVolatilityLabel: String = "Volatility"
    open val macroConsensusYoyLabel: String = "Consensus YoY"
    open val macroPriorYoyLabel: String = "Prior YoY"
    open val macroProOnly: String = "🔒 Pro Only"
    open val macroProHigh: String = "🔒 High"
    open val macroProLockedBadge: String = "🔒 Pro Locked"

    open val macroFedWatchTitle: String = "CME FedWatch: Rate Cut Odds"
    open val macroDxyTitle: String = "DXY: US Dollar Index"
    open val macroUs10yTitle: String = "US 10Y: 10-Year Treasury Yield"
    open val macroBrentTitle: String = "Brent Crude Oil"

    // Rainbow Chart Strings
    open val rainbowHeaderTitle: String = "Bitcoin Rainbow Chart"
    open val rainbowHeaderSubtitle: String = "Dynamic Rainbow Chart"
    open val rainbowFormulaTag: String = "Dynamic Power Law (94.3% R² Fit)"
    open val rainbowDescription: String = "A dynamic Power Law regression model fitting Bitcoin's entire price history since 2012."
    open val rainbowCurrentBandLabel: String = "Current Valuation Band"
    open val rainbowCyclePeakTarget: String = "Cycle Top Target"
    open val rainbowFireSaleTarget: String = "Fire Sale Floor"
    open val rainbowFilterAll: String = "All Bands"
    open val rainbowInspectHelp: String = "Touch & drag on chart to inspect historical price & bands"
    open val halvingVerticalLabel: String = "Halving"
    open val halving2032Est: String = "Halving 2032 (Est)"

    // Alt Season Strings
    open val altSeasonHeaderTitle: String = "Altcoin Season Index"
    open val altSeasonSubtitle: String = "75% of Top 50 coins vs Bitcoin over 90 days"
    open val altSeasonScoreLabel: String = "Altcoin Season Score"
    open val altSeasonBtcSeasonLabel: String = "Bitcoin Season"
    open val altSeasonNeutralLabel: String = "Neutral Zone"
    open val altSeasonAltSeasonLabel: String = "Altcoin Season"
    open val altSeasonOutperformedCount: String = "%d of %d coins beat BTC"
    open val topPerformers90d: String = "Top 90D Outperformers vs BTC"

    // Latest News Strings
    open val latestNewsTitle: String = "Latest News"
    open val latestNewsSubtitle: String = "Live curated headlines & market intelligence"

    // Fear & Greed Strings
    open val fearGreedHeaderTitle: String = "Fear & Greed Index"
    open val fearGreedSubtitle: String = "Market sentiment, euphoria & accumulation clock"
    open val fearGreedNow: String = "Now"
    open val fearGreedYesterday: String = "Yesterday"
    open val fearGreedLastWeek: String = "Last Week"
    open val fearGreedLastMonth: String = "Last Month"
    open val extremeFearLabel: String = "Extreme Fear"
    open val fearLabel: String = "Fear"
    open val neutralLabel: String = "Neutral"
    open val greedLabel: String = "Greed"
    open val extremeGreedLabel: String = "Extreme Greed"
    open val fearGreedAccumulationTip: String = "Historically, readings under 25 (Extreme Fear) represent optimal macro accumulation windows."

    // Pi Cycle Strings
    open val piCycleHeaderTitle: String = "Pi Cycle Indicators (Top & Bottom)"
    open val piCycleSubtitle: String = "Top (111SMA vs 350SMA×2) & Bottom (150EMA vs 471SMA×0.745)"
    open val piCycleTopSectionTitle: String = "🔴 PI CYCLE TOP (PEAK / SELL SIGNAL)"
    open val piCycleBottomSectionTitle: String = "🟢 PI CYCLE BOTTOM (FLOOR / BUY SIGNAL)"
    open val piCycleDma111: String = "111-Day SMA"
    open val piCycleDma350x2: String = "350-Day SMA × 2 (Top Threshold)"
    open val piCycleEma150: String = "150-Day EMA"
    open val piCycleSma471x0745: String = "471-Day SMA × 0.745 (Floor Threshold)"
    open val piCycleMa200w: String = "200-Week SMA Floor"
    open val piCycleDistanceToCross: String = "Distance to Pi Top Cross"
    open val piCycleDistanceToBottomCross: String = "Distance to Bottom Cross"
    open val piCycleStatusSafeText: String = "Accumulation & Expansion Phase (Safe Zone)"
    open val piCycleStatusAlertText: String = "🚨 PI CYCLE TOP CROSS DETECTED (Cycle Peak Risk)"
    open val piCycleBottomStatusSafeText: String = "Above Generational Bottom (Expansion Phase)"
    open val piCycleBottomStatusAlertText: String = "🚨 PI CYCLE BOTTOM CROSS DETECTED (Generational Floor Buy)"
    open val piCycleGenerationalSupport: String = "Generational Support Floor"

    // Futures Terminal Strings
    open val futuresTerminalTitle: String = "FUTURES TERMINAL"
    open val futuresOfficialFeedsSub: String = "Binance USDT-M Official Feeds · Zero Mock Data"
    open val futuresStateLive: String = "LIVE"
    open val futuresStateStale: String = "STALE"
    open val futuresStateOffline: String = "OFFLINE"
    open val futuresStateUnavailable: String = "UNAVAILABLE"
    open val futuresLastMarkIndexTitle: String = "LAST / MARK / INDEX PRICES"
    open val futuresLastTradedPrice: String = "Last Traded Price"
    open val futuresMarkPrice: String = "Mark Price"
    open val futuresIndexPrice: String = "Index Price"
    open val futuresBasis: String = "Basis (Mark - Index)"
    open val futuresHigh24h: String = "24h High"
    open val futuresLow24h: String = "24h Low"
    open val futuresVolume24h: String = "24h Volume (USDT)"
    open val futuresBboSpreadTitle: String = "BEST BID / ASK (BBO) & SPREAD"
    open val futuresBestBid: String = "BEST BID"
    open val futuresBestAsk: String = "BEST ASK"
    open val futuresOrderbookSpread: String = "Orderbook Spread:"
    open val futuresPerpFundingTitle: String = "PERPETUAL FUNDING & COUNTDOWN"
    open val futuresCurrentFundingRate: String = "Current Funding Rate"
    open val futuresNextFundingIn: String = "Next Funding in"
    open val futuresApproxAprLabel: String = "Approx APR (rate × 3 × 365):"
    open val futuresOpenInterestTitle: String = "OPEN INTEREST (15S POLLED)"
    open val futuresContracts: String = "contracts"
    open val futuresNoOpenInterest: String = "No open interest data"
    open val futuresFearGreed5m: String = "Fear & Greed (5m)"
    open val futuresBtcDominance60s: String = "BTC Dominance (60s)"
    open val futuresLiquidationsTapeTitle: String = "🚨 Binance Forced Orders (Liquidations)"
    open val futuresLiquidationsListening: String = "Listening to live Binance liquidation stream..."
    open val futuresCaptured: String = "captured"
    open val futuresLongLiq: String = "LONG RECT (SELL)"
    open val futuresShortLiq: String = "SHORT RECT (BUY)"
    open val futuresTradesTapeTitle: String = "⚡ Real-Time Trades Tape"
    open val futuresTradesAwaiting: String = "Awaiting trade flow..."
    open val futuresPriceHeader: String = "Price (USDT)"
    open val futuresQtyHeader: String = "Qty"
    open val futuresTimeHeader: String = "Time"
    open val futuresLockedTitle: String = "Pro Futures Pair Locked"
    open val futuresLockedDesc: String = "%s (%s) is part of the Pro tier. Unlock full institutional live order flow, basis arbitrage, and perpetual data."
    open val futuresLockedButton: String = "Unlock Pro Pair Access"
    open val futuresProPair: String = "Pro Pair"
    open val futuresSizePrefix: String = "Size:"
    open val futuresAllPairs: String = "All Pairs"
    open val futuresSearchPair: String = "Search crypto pair..."
    open val futuresSwitchPair: String = "Switch Pair"
    open val futuresPerpetualContract: String = "Perpetual Contract"

    // Market Intelligence & "Why is Asset Moving?" (ChatGPT 5.6 Luna Product Upgrade)
    open val intelligenceLayerTitle: String = "MARKET INTELLIGENCE LAYER"
    open val whyAssetMovingHeader: String = "Why is %s moving?"
    open val marketStateHeader: String = "MARKET STATE"
    open val confidenceHeader: String = "Confidence"
    open val driversHeader: String = "Primary Market Drivers"
    open val driverSpotDemand: String = "Spot Demand"
    open val driverOpenInterest: String = "Open Interest"
    open val driverFundingSkew: String = "Funding Skew"
    open val driverLiquidations: String = "Liquidation Pressure"
    open val driverMacroSentiment: String = "Macro Regime"
    open val interpretationLabel: String = "Interpretation: "
    open val cascadeRiskLabel: String = "Cascade Risk: "
    open val invalidationLabel: String = "Invalidation: "
    open val expandDeepDive: String = "Tap for Deep Dive & Invalidation"
    open val collapseDeepDive: String = "Hide Deep Dive"
    open val liquidationMapHeader: String = "AGGREGATED LIQUIDATION POOLS"
    open val liquidationMapSub: String = "Estimated liquidation clusters & sweep liquidity"
    open val shortLiquidationPools: String = "Short Liquidation Pools (Above Price)"
    open val longLiquidationPools: String = "Long Liquidation Pools (Below Price)"
    open val currentPriceAnchor: String = "CURRENT PRICE"
    open val estimatedPoolSuffix: String = "estimated liquidity"
    open val tabClustersMap: String = "Clusters Map"
    open val tabLiveStream: String = "Live Feed"

    // Market Intelligence Explainer Modal ("What is this & How it works")
    open val marketIntelligenceExplainerTitle: String = "Market Intelligence Layer: How it Works"
    open val marketIntelligenceInfoButton: String = "What is this?"
    open val marketIntelligenceConfidenceTitle: String = "Confidence Score (e.g. 79%)"
    open val marketIntelligenceConfidenceDesc: String = "Measures the quantitative strength of current futures positioning based on volume, open interest, and momentum."
    open val marketIntelligenceRegimesHeader: String = "Market Regimes (What the signals mean)"
    open val regimeShortSqueezeTitle: String = "Short Squeeze Pressure"
    open val regimeShortSqueezeDesc: String = "Heavy short positions are trapped. A minor upward move could trigger cascading liquidations and sharp spikes."
    open val regimeSpotAccumulationTitle: String = "Spot Accumulation"
    open val regimeSpotAccumulationDesc: String = "Real spot buying absorption rather than risky leverage. Signals healthy, sustainable market structure."
    open val regimeBalancedConsolidationTitle: String = "Balanced Consolidation"
    open val regimeBalancedConsolidationDesc: String = "Derivatives leverage and spot demand are in equilibrium, indicating range-bound trading."
    open val marketIntelligenceTelemetryHeader: String = "Key Telemetry"
    open val telemetryOpenInterestTitle: String = "Open Interest (OI)"
    open val telemetryOpenInterestDesc: String = "Total capital locked in active derivatives contracts."
    open val telemetryFundingRateTitle: String = "Funding Rate"
    open val telemetryFundingRateDesc: String = "Periodic fee between longs and shorts. Highly positive rates warn of overleveraged longs prone to flushes."
    open val marketIntelligenceUnderstood: String = "Understood"

    // Structured 3-Line Metric Explainers (Headers & Micro-copy)
    open val explainerWhatItIsLabel: String = "What it is: "
    open val explainerWhatItShowsLabel: String = "What it shows: "
    open val explainerNotASignalLabel: String = "Not a signal: "

    // Macro Metric Explainers
    open val explainerNfpWhat: String = "Monthly US employment change excluding farm workers, reported by the Bureau of Labor Statistics."
    open val explainerNfpShows: String = "Labor market tightness and economic activity directly shaping Federal Reserve rate decisions."
    open val explainerNfpNotMean: String = "Macro liquidity context for interest rates, not a direct crypto directional signal."

    open val explainerCpiWhat: String = "Headline inflation metric tracking average price changes across goods and services."
    open val explainerCpiShows: String = "Consumer purchasing power erosion and the path toward the Fed's 2% target."
    open val explainerCpiNotMean: String = "Inflation trajectory context, not an algorithmic market execution trigger."

    open val explainerFedWatchWhat: String = "Implied interest rate cut probability calculated from 30-day Fed Funds futures trading."
    open val explainerFedWatchShows: String = "Market consensus expectations for Federal Reserve monetary policy easing."
    open val explainerFedWatchNotMean: String = "Market probability distribution, not a guaranteed central bank policy commitment."

    open val explainerDxyWhat: String = "Geometrically weighted index measuring USD strength against 6 major fiat currencies."
    open val explainerDxyShows: String = "Global dollar liquidity conditions, capital flight, and relative dollar purchasing power."
    open val explainerDxyNotMean: String = "Macro liquidity tide, not an isolated determinant of crypto market bottoms or tops."

    open val explainerUs10yWhat: String = "Annual interest yield on 10-year United States government debt securities."
    open val explainerUs10yShows: String = "Risk-free global cost of capital and long-term economic growth/inflation expectations."
    open val explainerUs10yNotMean: String = "Macro discount hurdle rate for risk assets, not a short-term execution trigger."

    open val explainerBrentWhat: String = "International pricing benchmark for physical crude oil contracts."
    open val explainerBrentShows: String = "Headline commodity energy inflation pressures and global industrial activity."
    open val explainerBrentNotMean: String = "Upstream component of inflation indices, not a direct crypto price driver."

    // Futures Metric Explainers
    open val explainerFundingWhat: String = "Periodic funding fee exchanged between longs and shorts on perpetual contracts."
    open val explainerFundingShows: String = "Traders' positioning crowding and directional leverage bias across perpetual venues."
    open val explainerFundingNotMean: String = "Crowded skew reflects market leverage, not an immediate or guaranteed price reversal."

    open val explainerOiWhat: String = "Aggregate gross open positions and dollar valuation across active perpetual contracts."
    open val explainerOiShows: String = "Net inflow or outflow of derivatives capital and ongoing leverage expansion."
    open val explainerOiNotMean: String = "Measures outstanding derivatives commitment, not directional outcome without price context."

    open val explainerSentimentWhat: String = "Sentiment index (0-100) and BTC market capitalization share."
    open val explainerSentimentShows: String = "Broader market risk appetite skew and capital rotation between Bitcoin and Altcoins."
    open val explainerSentimentNotMean: String = "Broad market macro posture, not a short-term trade execution signal."

    open val explainerLiqWhat: String = "Forced position liquidations executed upon maintenance margin breach."
    open val explainerLiqShows: String = "Concentrated price nodes where over-leveraged accounts are closed by the exchange."
    open val explainerLiqNotMean: String = "Forced risk unwinding cascades, not organic institutional directional accumulation."

    // AI Market Analyst Assistant
    open val aiAssistantTitle: String = "AI Market Analyst"
    open val aiAssistantBadge: String = "LIVE CC"
    open val aiAssistantSubtitle: String = "Real-Time Cycle, Futures & Whale Intelligence"
    open val aiPromptPlaceholder: String = "Ask about BTC cycle, funding, support, altseason..."
    open val aiAskButton: String = "Analyze"
    open val aiQuickActionCycle: String = "Cycle & Halving Risk"
    open val aiQuickActionFutures: String = "Funding & Leverage Flow"
    open val aiQuickActionWhales: String = "Whale Accumulation"
    open val aiQuickActionAltseason: String = "Altseason Readiness"
    open val aiDisclaimer: String = "Powered by live Gemini AI models & real-time telemetry. Not financial advice."
    open val aiThinking: String = "Analyzing live telemetry & order book..."
    open val aiGreeting: String = "I'm here. What would you like to see?"
    open val aiClearHistory: String = "Clear Chat"
    open val aiLiveContextTag: String = "Live Feeds Attached"
    open val aiPromptPrefixCycle: String = "Analyze the current cycle phase, halving progress and risk/reward for the market."
    open val aiPromptPrefixFutures: String = "Interpret current futures funding rates, open interest, and liquidation risks."
    open val aiPromptPrefixWhales: String = "What are the latest whale alerts indicating for institutional accumulation or distribution?"
    open val aiPromptPrefixAltseason: String = "Assess whether market conditions, BTC dominance, and sentiment favor an Altseason right now."

    // CryptoCycle Design Suite Strings
    open val homeSubtitle: String = "Understand. Analyze. Trade Smarter."
    open val bullishState: String = "Bullish"
    open val bearishState: String = "Bearish"
    open val neutralState: String = "Neutral"
    open val highRiskBadge: String = "High Risk"
    open val moderateRiskBadge: String = "Moderate Risk"
    open val lowRiskBadge: String = "Low Risk"
    open val threeThingsThatMatter: String = "3 things that matter now"
    open val tabOverview: String = "Overview"
    open val tabCoins: String = "Coins"
    open val tabWatchlist: String = "Watchlist"
    open val tabSectors: String = "Sectors"
    open val totalMarketCapLabel: String = "Total Market Cap"
    open val volume24hLabel: String = "24h Volume"
    open val explainAction: String = "Explain >"
    open val spotBuyingLabel: String = "Spot Buying"
    open val fundingRateLabel: String = "Funding Rate"
    open val tabAnalytics: String = "Analytics"
    open val tabOnChain: String = "On-Chain"
    open val tabEvents: String = "Events"
    open val onChainNetworkHeader: String = "Network & On-Chain Intelligence"
    open val eventsMilestonesHeader: String = "Protocol Milestones & Catalysts"
    open val upcomingTag: String = "UPCOMING"
    open val completedTag: String = "COMPLETED"
    open val activeTag: String = "ACTIVE"
    open val whaleConcentrationLabel: String = "Whale Holder Concentration"
    open val exchangeNetFlowLabel: String = "Exchange Reserve Flow"
    open val netOutflowBias: String = "Net Outflows (Accumulation)"
    open val activeAddressesEstimate: String = "Active Network Wallets (24h)"
    open val cycleMomentumTitle: String = "Cycle Momentum & Relative Health"
    open val circulatingSupplyTitle: String = "Circulating Supply"
    open val maxSupplyLabel: String = "Max Supply"
    open val tabTopSignals: String = "Top Signals"
    open val tabMySignals: String = "My Signals"
    open val tabPerformance: String = "Performance"
    open val scoreLabel: String = "Score"
    open val timeframeLabel: String = "Timeframe"
    open val expectedScenarioLabel: String = "Expected Scenario"
    open val viewFullAnalysisBtn: String = "View Full Analysis"
    open val tabCourses: String = "Courses"
    open val tabLessons: String = "Lessons"
    open val tabQuizzes: String = "Quizzes"
    open val tabMyProgress: String = "My Progress"
    open val commonMistakeLabel: String = "Common mistake"
    open val backLabel: String = "Back"
    open val nextLabel: String = "Next"
    open val alertsTitle: String = "Alerts"
    open val createAlertBtn: String = "Create Alert"
    open val tabActive: String = "Active"
    open val tabHistory: String = "History"
    open val tabTemplates: String = "Templates"
    open val alertInsightLabel: String = "Alert Insight"
    open val alertInsightText: String = "This combination historically increases volatility by 62% within the next 24h."
    open val whyBtn: String = "Why?"
    open val marketCopilotTitle: String = "AI Market Data Analyst"
    open val betaBadge: String = "Beta"
    open val askAnythingMarket: String = "Ask anything about the market..."
    open val relatedDataLabel: String = "Related data"
    open val liquidationMapTab: String = "Liquidation Map"
    open val liquidationHeatmapTab: String = "Liquidation Heatmap"
    open val liquidationWarningText: String = "High liquidation clusters detected. Increased volatility risk if support breaks."
    open val exchangeBinance: String = "Binance"
    open val backtestBadgeText: String = "90D Verified Backtest: 72.8% Win Rate | Profit Factor: 2.14"
    open val derivativesRiskGuardrail: String = "Derivatives Protocol: Trade responsibly. Never risk more than 1-2% equity per position."
}

val EnglishStrings: AppStrings = AppStrings(AppLanguage.ENGLISH)
val GreekStrings: AppStrings = GreekAppStrings()
val GermanStrings: AppStrings = GermanAppStrings()
val FrenchStrings: AppStrings = FrenchAppStrings()
val SpanishStrings: AppStrings = SpanishAppStrings()
val ItalianStrings: AppStrings = ItalianAppStrings()

fun getAppStrings(language: AppLanguage): AppStrings {
    return when (language) {
        AppLanguage.ENGLISH -> EnglishStrings
        AppLanguage.GREEK -> GreekStrings
        AppLanguage.GERMAN -> GermanStrings
        AppLanguage.FRENCH -> FrenchStrings
        AppLanguage.SPANISH -> SpanishStrings
        AppLanguage.ITALIAN -> ItalianStrings
    }
}

fun CoinCategory.getLocalizedName(strings: AppStrings): String {
    return when (this) {
        CoinCategory.ALL -> strings.catAll
        CoinCategory.LAYER1 -> strings.catLayer1
        CoinCategory.LAYER2 -> strings.catLayer2
        CoinCategory.DEFI -> strings.catDefi
        CoinCategory.AI_INFRA -> strings.catAiInfra
        CoinCategory.MEME -> strings.catMeme
        CoinCategory.RWA_DEPIN -> strings.catRwaDepin
        CoinCategory.UTILITY -> strings.catUtility
        CoinCategory.FAVORITES -> strings.catFavorites
    }
}

val LocalAppStrings = compositionLocalOf { EnglishStrings }
